...
/*        if (CancelDraw.action.triggered)
        ...
        //If we were tracking the cursor when the cancel measurement action is raised
        //we need to remove the last section of the annotation
        if (CancelDraw.action.triggered)
        {
            //remove the last line if needed
            int count = doodle.sections[0].Count;
            if (trackCursor && count > 1)
            {
                doodle.sections[0][count - 1] = doodle.sections[0][count - 2];
                UpdateDoodle();
            }

            //and cancel measuring
            ToggleMeasuring();
            return;
        }
